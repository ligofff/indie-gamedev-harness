...
export function installHarness({ root, flags = {}, packageVersion, dryRun = false }) {
  ...
  if (manifest) return updateHarness({ root: projectRoot, packageVersion, force: Boolean(flags.force), dryRun });
  ...
  try {
    ...
    return { dryRun, copied, adopted, configChanged: merged.changed, validation };
  } catch (error) {
