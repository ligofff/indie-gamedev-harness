...
const variantFlags = new Map([["orchestrator-variant", "orchestrator"], ["lead-programmer-variant", "lead-programmer"], ["creative-variant", "creative-guy"], ["explorer-variant", "explorer"], ["simple-programmer-variant", "simple-programmer"]]);
...
export function parseCliArguments(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (argument.startsWith("--")) {
      ...
      if (variantFlags.has(flag)) {
        const variant = argv[index + 1];
        if (!variant || variant.startsWith("--")) throw invalidArgument(`Missing value for --${flag}`);
        result.flags.variants ||= {};
        result.flags.variants[variantFlags.get(flag)] = variant;
        index += 1;
        continue;
      }
  ...
  if ((result.flags.models || result.flags.variants) && !["install", "configure-models"].includes(result.command)) throw invalidArgument("Model and variant flags require install or configure-models");
  if (result.flags["skip-models"] && (result.flags.models || result.flags.variants)) throw invalidArgument("--skip-models cannot be combined with model or variant flags");
  if (result.flags.variants && Object.keys(result.flags.variants).some((role) => !(role in (result.flags.models || {})))) throw invalidArgument("Variant flags require matching model flags");
  return result;
}
...
function previousVariants(root) {
  const config = readOpenCodeConfig(root).config;
  return Object.fromEntries(CANONICAL_ROLES.map((role) => [role, config.agent?.[role]?.variant]).filter(([, variant]) => variant));
}

async function selectModels(root, flags, io, discover = discoverOpenCodeModels) {
  if (flags["skip-models"]) return { models: {}, variants: {} };
  if (flags["non-interactive"]) return { models: Object.fromEntries(CANONICAL_ROLES.map((role) => [role, flags.models?.[role] || "inherit"])), variants: flags.variants || {} };
  let discovered = { models: [], variantsByModel: {} };
  try { discovered = await discover({ cwd: root }); }
  catch (error) {
  ...
  const assignments = await promptForModelAssignments({ models: discovered, previousAssignments: previousModels(root), previousVariants: previousVariants(root), input: process.stdin, output: io.stdout, createInterface, yes: Boolean(flags.yes) });
  for (const role of Object.keys(flags.models || {})) {
    if (!(role in (flags.variants || {}))) delete assignments.variants[role];
  }
  return { models: { ...assignments.models, ...flags.models }, variants: { ...assignments.variants, ...flags.variants } };
}
...
export async function runCli(argv, io, { discoverOpenCodeModels: discover = discoverOpenCodeModels } = {}) {
  ...
  if (parsed.flags.help || parsed.command === null) {
    io.stdout.write("Usage: gamedev-harness <install|configure-models|update|uninstall|status|validate> [path] [--yes] [--dry-run] [--force] [--all] [--set-default] [--skip-models] [--non-interactive] [--orchestrator-model provider/model] [--lead-programmer-model provider/model] [--creative-model provider/model] [--explorer-model provider/model] [--simple-programmer-model provider/model] [--orchestrator-variant variant] [--lead-programmer-variant variant] [--creative-variant variant] [--explorer-variant variant] [--simple-programmer-variant variant]\n");
    return 0;
  }
  try {
    ...
    if (parsed.command === "install") {
      ...
      io.stdout.write(`install: ${result.dryRun ? "dry-run" : "ok"}; ${result.reinstalled ? "reinstalled" : `copied ${result.copied}; adopted ${result.adopted}`}\n`);
