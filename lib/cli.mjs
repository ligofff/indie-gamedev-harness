...
async function selectModels(root, flags, io) {
  ...
  let models = [];
  try { models = await discoverOpenCodeModels(); }
  catch { io.stderr.write("Model discovery failed; choose inherit, manual model IDs, or skip.\n"); }
...
export async function runCli(argv, io) {
  ...
  try {
    ...
    if (parsed.command === "install") {
      const models = await selectModels(root, parsed.flags, io);
