...
export function mergeModelAssignments(text, assignments) {
  ...
  if (Object.keys(assignments).some((role) => !CANONICAL_ROLES.includes(role))) {
  ...
  return { text: output, changed: output !== text, modelsSetByInstaller };
}
...
export function mergeHarnessConfig(text, options = {}) {
  ...
  return {
    text: output,
    changed: output !== text,
    changes: { schemaAdded, instructionAdded, defaultAgentSetByInstaller, modelsSetByInstaller: {}, permissionsAdded },
    permissionConflicts,
  };
}
