...
export async function promptForModelAssignments({ models: discovered = [], variantsByModel = {}, previousAssignments = {}, previousVariants = {}, input = process.stdin, output = process.stdout, createInterface = nodeCreateInterface, yes = false } = {}) {
  ...
  try {
    ...
    for (const role of CANONICAL_ROLES) {
      ...
      if (model === "inherit" || !models.includes(model) || !available?.length) continue;
      ...
      while (true) {
        ...
        if (!variant) { variants[role] = fallback; break; }
        if (variant.toLowerCase() === "inherit") { variants[role] = "inherit"; break; }
        if (available.includes(variant)) { variants[role] = variant; break; }
